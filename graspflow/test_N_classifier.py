...
pcs_env = pcs_env[0,kkk].unsqueeze(0)
qs = qs[0,kkk].unsqueeze(0)
...
model = taskgrasp_refine.load_gcn_model()
# retain the _t tensors here and load
query = read_query("query.txt", n_envs=ts.shape[0], n_grasps=ts.shape[1])
query = query.flatten()
intents = np.array(taskgrasp_refine.get_intents(query = query))
pc = pc.squeeze(0)
print(_t.shape, qs.shape, pc.shape, query.shape)
logit, scores = taskgrasp_refine.evaluate(model, _t, qs, pc, intents)
...
pc_env_mesh = trimesh.points.PointCloud( pcs_env[0].detach().cpu().numpy(), colors =[0,255,0,50])
pc_mesh = trimesh.points.PointCloud( pc[0].detach().cpu().numpy(), colors =[0,0,255,50])
scene.add_geometry(pc_env_mesh)
scene.add_geometry(pc_mesh)

for i in range(_t.shape[0]):
    g = construct_grasp_matrix(qs[i].detach().cpu().numpy(), _t[i].detach().cpu().numpy())
    #scene.add_geometry(gripper_bd(scores), transform=g[0])
...
quat_inv = quaternion.quat_inv(qs)
...
# scene.add_geometry(gripper_bd(scores))
scene.show()
