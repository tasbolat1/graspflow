...
from graspflow3 import GraspFlow
...
parser.add_argument("--use_svgd", action='store_true', help='If set, uses SVGD to refine', default=False)
...
for i in range(num_unique_pcs):
    ...
    if args.method == "graspnet":
        ...
        ts_refined, qs_refined, info = graspflow.refine_grasps_GrapsFlow_test(qs[i], ts_normalized[i], pcs_normalized[i], pc_means[i], classifiers=args.classifier)
