...
from networks.utils import save_model, add_weight_decay, quat_to_A_vec
import time
...
# python train_evaluator.py --data_dir_grasps data/grasps_new/preprocessed --data_dir_pcs data/pcs --batch_size 256 --lr 0.0001 --device_ids 0 1
...
train_dataset = GraspDataset(path_to_grasps = 'data/grasps_new/preprocessed',
                             path_to_grasps_tight='data/grasps_tight/preprocessed',
                             path_to_pc=args.data_dir_pcs, split='train', augment=True)
...
test_dataset = GraspDataset(path_to_grasps = 'data/grasps_new/preprocessed',
                             path_to_grasps_tight='data/grasps_tight/preprocessed',
                             path_to_pc=args.data_dir_pcs, split='test', augment=False)
test_dataloader = DataLoader(test_dataset, batch_size=args.batch_size+600, shuffle=False, num_workers=args.num_workers)
...
def train(epoch):
    ...
    for k, (quat, trans, pcs, labels, cats) in enumerate(train_dataloader):
        ...
        # forward pass
        eval_out = model(A_vec, trans, pcs)
