...
parser.add_argument('--cfg', type=str, help='Path to config file', default='configs/graspflow_isaac_params.yaml')
...
for i in range(num_unique_pcs):
    if args.method == 'GraspFlow':
        ts_refined, qs_refined, info = graspflow.refine_grasps_GraspFlow(qs[i], ts_normalized[i], pcs_normalized[i], pc_means[i], pc_envs=pcs_env[i], queries=query[i], classifiers=args.classifier)
    elif args.method == 'GraspOpt':
        ts_refined, qs_refined, info = graspflow.refine_grasps_GraspOpt(qs[i], ts_normalized[i], pcs_normalized[i], pc_means[i], pc_envs=pcs_env[i], queries=query[i])
    elif args.method == 'GraspOptES':
    ...
    # total_init_grasps_translations[i,:,:] = info.data['translations'][-1,...]
    # total_init_grasps_quaternions[i,:,:] = info.data['quaternions'][-1,...]
